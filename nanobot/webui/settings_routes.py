...
from nanobot.webui.settings_api import (
    ...
    update_skill_governance_settings,
    update_student_mode_settings,
...
class WebUISettingsRouter:
    ...
    async def dispatch(self, connection: Any, request: WsRequest, path: str) -> Response | None:
        ...
        if path == "/api/settings/skill-governance/update":
    ...
    def _handle_settings_skill_governance_update(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
            payload = update_skill_governance_settings(self._query(request))
        except WebUISettingsError as e:
            return self._error_response(e.status, e.message)
        return self._json_response(self._with_restart_state(payload, section="runtime"))

    def _handle_settings_student_mode_update(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
            payload = update_student_mode_settings(self._query(request))
        except WebUISettingsError as e:
            return self._error_response(e.status, e.message)
        return self._json_response(self._with_restart_state(payload, section="runtime"))
