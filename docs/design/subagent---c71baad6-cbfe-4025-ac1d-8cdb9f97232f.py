...
class SubagentManager:
    ...
    def _build_tools(
        ...
        # Profile tool allow-list: unregister everything not explicitly allowed.
        if profile is not None and profile.tools is not None:
            allowed = set(profile.tools) | {"spawn"}  # spawn handled below
            for name in list(registry._tools.keys()):
                if name not in allowed:
                    registry.unregister(name)

        # Recursive spawn control: depth limit + per-profile permission.
        allow_spawn = (
    ...
    async def spawn(
        ...
        if self.profiles and profile and profile not in self.profiles:
            valid = ", ".join(self.profiles)
            return (
                f"Error: unknown profile '{profile}'. Choose one of: {valid}."
            )
        ...
        try:
            ...
            if result.stop_reason == "tool_error":
                ...
                if expected_output and len(final_result.strip()) < 20:
                    # Clearly insufficient result: demote to error so the main
                    # agent can decide to re-delegate with a better task spec.
    ...
    def _build_subagent_prompt(
        ...
        # Preload profile skills as full content (not just the summary).
        preloaded_skills = ""
