...
export NANOBOT_CONFIG="${NANOBOT_CONFIG:-$SCRIPT_DIR/.local/config.japanese-sentence-coach.json}"
export NANOBOT_WORKSPACE="${NANOBOT_WORKSPACE:-$SCRIPT_DIR/.local/workspace-japanese-sentence-coach}"
...
if [[ -z "${LEARN_JP_TELEGRAM_BOT_TOKEN:-}" ]]; then
  echo "LEARN_JP_TELEGRAM_BOT_TOKEN is empty." >&2
  echo "Edit $NANOBOT_ENV_FILE and set LEARN_JP_TELEGRAM_BOT_TOKEN before starting Japanese Sentence Coach." >&2
  exit 1
fi
