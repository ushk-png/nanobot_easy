...
export NANOBOT_CONFIG="${NANOBOT_CONFIG:-$SCRIPT_DIR/.local/config.rees.json}"
export NANOBOT_WORKSPACE="${NANOBOT_WORKSPACE:-$SCRIPT_DIR/.local/workspace-rees}"
...
if [[ -z "${REES_TELEGRAM_BOT_TOKEN:-}" ]]; then
  echo "REES_TELEGRAM_BOT_TOKEN is empty." >&2
  echo "Edit $NANOBOT_ENV_FILE and set REES_TELEGRAM_BOT_TOKEN before starting Rees." >&2
  exit 1
fi
